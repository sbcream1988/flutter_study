...
import 'package:stack_app/data/csv/intraday_info_parser.dart';
import 'package:stack_app/data/mapper/company_mapper.dart';
...
class StockRepositoryImpl implements StockRepository {
  ...
  Future<Result<CompanyInfo>> getCompanyInfo(String symbol) async {
    try {
      final dto = await _api.getCompnyInfo(symbol: symbol);
  ...
  @override
  Future<Result<List<IntradayInfo>>> getIntrdayInfo(String symbol) async {
    try {
      final response = await _api.getIntradayInfo(symbol: symbol);
      final result = await _IntradayInfoParser.parse(response.body);
      return Result.success(result);
    } catch (e) {
      return Result.error(Exception('intrday 정보 로드 실패!! : ${e.toString()}'));
    }
  }
}
